...
from PyQt5.QtCore import Qt, QSize, QPoint, QTimer, QThread, pyqtSignal, QMetaObject, pyqtSlot, QDateTime, QObject
...
class AsyncWorker(QObject):
    ...
    async def async_api_request(self, method, endpoint, data=None):
    ...
    def run_async_task(self, method, endpoint, data=None):
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        result = loop.run_until_complete(self.async_api_request(method, endpoint, data))
        loop.close()
        if result:
            self.finished.emit(result)
        else:
            self.error.emit("API request failed")
...
class TradingApp(QMainWindow):
    def __init__(self):
        ...
        self.API_BASE_URL = "http://api.rashedc.com"
        
        self.last_tp_table_update = 0
        self.tp_table_update_interval = 60  # in seconds

        # Async worker for API requests
        self.async_worker = AsyncWorker(self.API_BASE_URL)
        self.async_worker.finished.connect(self.on_sync_finished)
        self.async_worker.error.connect(self.on_sync_error)
    ...
    def load_active_orders(self):
        ...
        if os.path.exists(self.orders_file):
            ...
            print(f"{self.orders_file} not found. Starting with empty orders and TP levels.")
            self.active_orders = {}
            self.tp_levels = {}

        # Update GUI with local data
    ...
    def background_sync(self):
        ...
        self.async_worker.run_async_task('POST', '/api/place_order', data_to_sync)

    def on_sync_finished(self, result):
        if result.get('success'):
            print("Data synchronized with API successfully")
            # Update local data with synced data
            self.active_orders = result.get('active_orders', self.active_orders)
            self.tp_levels = result.get('tp_levels', self.tp_levels)
            # Update GUI with synced data
            #self.update_ui_from_loaded_data()
            self.update_gui_with_data()
        else:
            print("Failed to synchronize data with API")

    def on_sync_error(self, error_msg):
        print(f"Error during synchronization: {error_msg}") 
    ...
    #         'tp_levels': self.tp_levels
    #     }
    #     api_response = self.api_request('POST', '/api/place_order', data_to_save)
    ...
    #     api_response = self.api_request('POST', '/api/place_order', data_to_save)
    #     if api_response:
    #         print("Orders saved to API successfully")
    #     else:
    #         print("Failed to save orders to API")
    ...
    #         self.tp_levels = {}
    #     # Load from API
    #     api_response = self.api_request('GET', '/api/get_active_orders')
    #     if api_response:
    #         print("\nOrders loaded from API:")
    #         print(f"Active orders: {api_response.get('active_orders', {})}")
    #         print(f"TP levels: {api_response.get('tp_levels', {})}")
    #     else:
    #         print("Failed to load orders from API")
    ...
    def update_gui_with_data(self):
        # Update all relevant GUI components with current data
        self.update_trade_status()
        self.populate_tp_table()
        self.update_tp_table()
        # Add any other GUI updates needed


    def update_trade_status(self, skip_timer_start=False):
        current_ticker = self.ticker_combo.currentText()
        status_changed = False  # Flag to track if status has changed
        ...
        if current_ticker in self.active_orders:
            ...
            if self.trade_status_label.text() != status_text:
                status_changed = True
                self.trade_status_label.setText(status_text)
                self.trade_status_label.setStyleSheet("color: green;")
            ...
            if self.trade_status_label.text() != "Status: Not in trade":
                status_changed = True
                self.trade_status_label.setText("Status: Not in trade")
                self.trade_status_label.setStyleSheet("color: red;")
        ...
        if status_changed:
            print(f"Updated trade status for {current_ticker}: {'In trade' if current_ticker in self.active_orders else 'Not in trade'}")
            #self.async_sync_with_api()  # Sync with API when trade status changes
    ...
    #         self.save_active_orders()  # This will save locally and sync with API
    #         self.update_tp_table()
    ...
    def update_tp_level(self, row, column, new_value):
        ...
        if current_ticker in self.tp_levels and row < len(self.tp_levels[current_ticker]):
            ...
            #self.async_sync_with_api()
        
    #     # Update TP level in API
    #     # api_response = self.api_request('PUT', f'/api/save_tp_level/{current_ticker}/{row}', {
    #     #     'column': column,
    #     #     'value': new_value
    #     # })
    #     # if api_response:
    #     #     print(f"TP level updated in API: Ticker={current_ticker}, Row={row}, Column={column}, New Value={new_value}")
    #     # else:
    #     #     print(f"Failed to update TP level in API")
    #     print(f"Updated TP level: Ticker={current_ticker}, Row={row}, Column={column}, New Value={new_value}")
    ...
    def clear_trade(self, ticker=None):
        ...
        self.update_response_area(f"Trade cleared for {ticker}.\n")
            ...
            # Clear trade in API
        api_response = self.api_request('POST', f'/api/clear_trade/{ticker}')
        if api_response:
            print(f"Trade cleared in API for {ticker}")
        else:
            print(f"Failed to clear trade in API for {ticker}")
    ...
    def send_order(self, action):
        ...
        api_response = self.api_request('POST', '/api/place_order', order)
        if api_response:
            response_text += "Order sent to API successfully.\n"
        else:
            response_text += "Failed to send order to API.\n"
